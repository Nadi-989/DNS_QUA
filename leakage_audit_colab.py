...
NOTEBOOK VERSION - paste this whole cell into Colab and press run.
Edit the CONFIG block below; there are no command-line arguments.

Addresses Reviewer 1 comment 10 (deterministic majority-label upper bound) and
Reviewer 2 comment 2 (overlap statistics before/after splitting, group-disjoint
split, and the separation of preprocessing / duplicate / entity-level leakage).
"""

# ============================== CONFIG ==============================
CSV_PATH    = "merged_heavy.csv"   # "merged_mal_clean.csv" for BCCC-2024
LABEL_COL   = "label"
ROUND_DEC   = 6      # None = exact equality only; 6 = also report near-duplicate equivalence
ENTITY_COL  = None   # e.g. "domain" / "capture_session" / "src_ip" if such a column exists
GROUPED_EVAL = False # True = also retrain the pipeline under both splits (slow)
TEST_SIZE   = 0.2
# ====================================================================
...
# ----------------------------------------------------------------------------- run
def run():
    df, Xdf, y = load(CSV_PATH, LABEL_COL)
    X = Xdf.to_numpy(dtype=np.float64)
    print(f"Loaded {CSV_PATH}: {X.shape[0]:,} rows x {X.shape[1]} numeric features; "
          f"attack rate {y.mean():.4f}")

    results = {"file": CSV_PATH, "seed": SEED, "rounding": ROUND_DEC}
    ...
    if ROUND_DEC is not None:
        gid_round = group_ids(X, decimals=ROUND_DEC)
        ceil_round = deterministic_ceiling(gid_round, y)
        show(f"1b. Same audit under equality after rounding to {ROUND_DEC} decimals", ceil_round)
    ...
    if ENTITY_COL:
        if ENTITY_COL not in df.columns:
            print(f"\n[warn] entity column '{ENTITY_COL}' not in file - skipped")
        else:
            ent = entity_overlap(df[ENTITY_COL], tr, te)
            show(f"4. Entity-level overlap on '{ENTITY_COL}' under the random split", ent)
