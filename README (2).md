...
`artifacts/manifest.json`, together with the class counts. The prepared corpora are:
...
The full SHA-256 digests are in `artifacts/manifest.json` rather than duplicated here, so that the values a
...
for BCCC-2024) are listed in full under `excluded_columns` in `artifacts/manifest.json` and in
`artifacts/features_*.json`.

### 2.4 Retained feature list

The final feature sets are written to `artifacts/features_CIC_Bell_DNS_EXF_2021.json` and
`artifacts/features_BCCC_CIC_Bell_DNS_2024.json`.
...
* **Split:** stratified 80/20 train/test. The exact row indices are released as
  `artifacts/train_indices_<dataset>.npy` and `artifacts/test_indices_<dataset>.npy`, so the partition can be
...
* **Cross-validation:** stratified five-fold, shared across all compared configurations (the paired protocol
  of Section 4.8). Fold indices are released as `artifacts/cv_folds_<dataset>.npz`. This is the **single
